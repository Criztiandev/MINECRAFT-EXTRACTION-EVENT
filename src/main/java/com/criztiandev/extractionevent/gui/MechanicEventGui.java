...
public class MechanicEventGui implements Gui {
    ...
    public MechanicEventGui(ExtractionEventPlugin plugin, LevRegion region) {
        this.plugin = plugin;
        this.region = region;
        this.inventory = Bukkit.createInventory(this, SIZE, "§8Mechanic: §d" + region.getId());
        render();
    }

    private void render() {
        for (int i = 0; i < SIZE; i++) inventory.setItem(i, filler());
        inventory.setItem(22, makeItem(Material.BARRIER, "§cComing Soon", "§7Mechanic features are under development."));
